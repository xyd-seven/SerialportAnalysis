...
class NetworkWorker(QThread):
    ...
    def _process_tx_queue(self):
        while not self.tx_queue.empty():
            try:
                ...
                elif self.mode == "TCP Server" and self.client_sock:
                    self.client_sock.sendall(data)
                elif self.mode == "UDP" and getattr(self, 'sock', None) and getattr(self, 'client_sock', None):
                    self.sock.sendto(data, self.client_sock)
            except queue.Empty:
                break
            except Exception as e:
                self.error_occurred.emit(f"发送异常: {str(e)}")
    ...
    def send_data(self, data_bytes):
        try:
            if self.mode == "TCP Client" and getattr(self, 'sock', None):
                self.tx_queue.put(data_bytes)
                return True, ""
            elif self.mode == "TCP Server":
                ...
                if getattr(self, 'sock', None) and getattr(self, 'client_sock', None):
                    self.tx_queue.put(data_bytes)
                    return True, ""
...
class StreamParser:
    ...
    def feed(self, raw_text):
        ...
        while True:
            ...
            # 强制拦截超过 2048 字节的无效包，极速查找下一个合法的包头！
            if total_len > 2048:
                next_idx = self.buffer.find(self.SYNC_HEADER, 1)
                if next_idx != -1:
                    self.buffer = self.buffer[next_idx:]
                else:
                    self.buffer = self.buffer[-len(self.SYNC_HEADER)+1:] if len(self.SYNC_HEADER) > 1 else bytearray()
                continue
...
class EcuMainWindow(QMainWindow):
    def __init__(self):
        ...
        self.all_frames = deque(maxlen=100000)
        ...
        # 🌟 修复：使用 deque 真正实现高性能 10 万条容量无缝丢弃！
        self.terminal_history = deque(maxlen=100000)
    ...
    def _load_older_history(self):
        ...
        older_history = list(self.terminal_history)[start_idx:end_idx]
    ...
    def open_regex_config(self):
        ...
        if dialog.exec():
            ...
            if hasattr(self.raw_log_console, 'highlighter'):
                # ==========================================
                # 🌟 核心调优 1：先清空屏幕！
                # 让底层的 rehighlight 去处理 0 行数据，彻底消除遍历卡顿
                # ==========================================
    ...
    def redraw_terminal_history(self):
        try:
            ...
            # ==========================================
            MAX_RENDER = 2000
            render_history = list(self.terminal_history)[-MAX_RENDER:]
    ...
    def on_serial_data_received(self, raw_bytes):
        ...
        if parsed_frames:
            ...
            if len(self.filtered_frames) > MAX_TABLE_ROWS:
                if target_type == "ALL" or not target_type:
                    self.filtered_frames = list(self.all_frames)
                else:
